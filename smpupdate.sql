...
CREATE OR REPLACE VIEW v_trucks AS SELECT 
	CAST(CASE WHEN (SELECT s.State FROM samples s WHERE s.SID = `trucks`.SID LIMIT 1) & 8 > 0 THEN CASE WHEN (SELECT s.a_noUploadFlag FROM samples s WHERE s.SID = `trucks`.SID LIMIT 1) = 2 THEN `trucks`.TID ELSE NULL END ELSE NULL END AS CHAR(50)) TID,
	`trucks`.`SID` AS `SID`,
	`trucks`.`MID` AS `MID`,
	`trucks`.`LPR` AS `LPR`,
	CAST(CASE WHEN (SELECT s.State FROM samples s WHERE s.SID = `trucks`.SID LIMIT 1) & 8 > 0 THEN CASE WHEN (SELECT s.a_noUploadFlag FROM samples s WHERE s.SID = `trucks`.SID LIMIT 1) = 2 THEN `trucks`.LPN ELSE NULL END ELSE NULL END AS CHAR(50)) LPN,
