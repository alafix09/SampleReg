...
void __fastcall TPwdForm::OKButtonClick(TObject *Sender)
{
	...
	else
	{
		...
		result = true;
		FDQuery->SQL->Text = "ALTER TABLE trucks ADD COLUMN a_forStorage INT NOT NULL DEFAULT 0";
		try {
			FDQuery->ExecSQL();
		}
		catch (...)
		...
		result = true;
		FDQuery->SQL->Text = "DROP INDEX samples_idx2 ON samples";
		try {
			FDQuery->ExecSQL();
		}
		catch (...)
		...
		result = true;
		FDQuery->SQL->Text = "CREATE INDEX samples_idx3 ON samples (CT, State, a_noUploadFlag)";
		try {
			FDQuery->ExecSQL();
		}
		catch (...)
		...
		result = true;
		FDQuery->SQL->Text = "CREATE INDEX samples_idx4 ON samples (CT, a_noUploadFlag, State)";
		try {
			FDQuery->ExecSQL();
		}
		catch (...)
